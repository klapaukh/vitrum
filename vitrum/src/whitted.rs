...
pub fn trace<T:Plane<Face>>(ray: &Ray, model: &T, lights: &Vec<Vector3D>,
...
fn trace_down<T:Plane<Face>>(ray: &Ray, model: &T, lights: &Vec<Vector3D>,
    i_a: f32, k_d: f32, k_s: f32, k_t: f32, depth: u8) -> f32 {
        ...
        match hit {
            Some(c) => {
                ...
                let contact = c.contact_point + contact_shift_factor * c.object.normal;
                let normal = c.object.normal;
